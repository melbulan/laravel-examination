...
<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">
            {{ __('List of Property Types') }}
        </h2>
    </x-slot>
    <div class="max-w-4xl mx-auto mt-12">

        <div class="flex justify-end mt-10">
            <a href="{{ route('property-types.create') }}" class="px-2 py-1 rounded-md bg-blue-500 text-sky-100 hover:bg-blue-700">+ Add New Property Type</a>
        </div>
            ...
            <div class="flex flex-col">
                <div class="inline-block min-w-full overflow-hidden align-middle border-b border-gray-200 shadow sm:rounded-lg">
                    ...
                    <table class="min-w-full">
                        ...
                        @forelse ($propertyTypes as $types)
                        <tr>
                            <td class="px-7 whitespace-no-wrap border-b border-gray-200 bg-gray-50">{{ $types->name }}</td>
                            <td class="px-7 whitespace-no-wrap border-b border-gray-200 bg-gray-50">{{ $types->description }}</td>
                            <td class="px-7 whitespace-no-wrap border-b border-gray-200 bg-gray-50">{{ $types->is_active ? 'Active': 'Inactive' }}</td>
                            <td class="px-7 whitespace-no-wrap border-b border-gray-200 bg-gray-50">
                                <form action="{{ route('property-types.destroy',$types->id) }}" method="POST">
                
                                    <a class="text-indigo-600 hover:text-indigo-1000" href="{{ route('property-types.show',$types->id) }}">
                                    ...
                                    <a href="{{ route('property-types.edit',$types->id) }}" class="text-green-600 hover:text-indigo-1000 text-gray-600">
                                        <svg xmlns="http://www.w3.org/2000/svg" class="w-6 h-6 inline-block" fill="none" viewBox="0 0 24 24" stroke="currentColor">
